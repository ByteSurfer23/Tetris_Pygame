...
def draw_next_shape(shape, screen):#displays the next block in tetris so that the user can see and decide his moves 
    font=pygame.font.SysFont('freesansbold.ttf',30)
    label=font.render('Next Shape',1,(255,255,255))
    sx=top_left_x+play_width+50#gets the posn where the text next shape must be displayed
    sy=top_left_y+play_height/2-100
    format=shape.shape[shape.rotation % len(shape.shape)]
    #this nested loop figures out the grid coords of the 0 and uses them to draw it from the sx and sy coords by multiplying with blocksize
    for i,line in enumerate(format):
        row=list(line)
        for j,column in enumerate(row):
            if column=="0":
                pygame.draw.rect(screen,shape.colour,(sx+j*block_size,sy+i*block_size,block_size,block_size),0)  
